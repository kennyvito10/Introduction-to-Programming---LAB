...
if len(singleRooms) > 5:
      raise Exception('Items exceeds the maximum allowed length of 5')
doubleRooms = []
...
class CheckIn(Hotel):                                                     #Making a CheckIn class for defining check in functions for two categories of rooms
    def inSingle(self):
        if len(singleRooms) == 5:
                return singleRooms.append(self.name)
        else:
            print("Sorry, rooms are full right now, please choose another category")
    def inDouble(self):
        if len(doubleRooms) == 5:
            return doubleRooms.append(self.name)
        else:
            print("Sorry, rooms are full right now, please choose another category")
...
while restart != ('N'):
    ...
    if usrInput == "1":
        ...
        if catInput == "single":
            i = input("Enter Name : ")
            runin(i).inSingle()
            print("Have a Pleasant Stay, ",i,"!")
        if catInput == "double":
            ii = input("Enter Name : ")
            runin(ii).inDouble()
            print("Have a Pleasant Stay, ",ii,"!")
